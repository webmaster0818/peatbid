...
def render_takaku_uru(b):
    ...
    return {
        ...
        "sections": [
            ("コツ1: 付属品をすべて揃える",
             f"{name}の査定では、外箱・冊子・カートン・ホログラムシールの有無で**査定額が10〜20%変動**します。市場相場（Yahoo中央値）を基準として、付属品なしの場合は市場相場の80〜90%程度まで下がることが一般的です。\\n\\n外箱の状態も重要で、シミ・破れ・水濡れ跡があると更に-5〜10%。購入時の状態を維持する努力が、最終的に数万〜数十万円の差を生みます。\\n\\nまず家中を徹底的に探し、すべての付属品を集めてから売却を検討してください。"),
            ("コツ2: 未開封の状態を維持",
...
def render_nisemono_mikata(b):
    ...
    return {
        ...
        "intro": f"{name}のような高額ウイスキーは、贋作リスクが高まる銘柄です。特に未開封・箱付きで市場相場（Yahoo中央値）前後の流通価格を持つ銘柄は、海外を中心に巧妙な贋作が出回ることもあります。本記事では、{name}の偽物を見抜く5つのチェックポイントと、不審なボトルへの対処法を解説します。",
        "sections": [
            ("贋作市場の実情",
...
def render_kihaku(b):
    ...
    return {
        ...
        "h1": f"{name}の希少性と投資価値",
        "toc_label": "希少性・投資価値",
        "intro": f"{name}は希少度{rarity_label}クラスのボトルで、現在の買取相場は約市場相場（Yahoo中央値）前後。投資対象として保有すべきか、それとも売却すべきか——コレクターが直面する判断を、データと市場動向から考察します。",
        "sections": [
            ("希少性のランク評価",
...
def render_hako_nashi(b):
    ...
    return {
        ...
        "sections": [
            ("箱なしによる査定額の影響",
             f"{name}の場合、外箱の有無で査定額が**10〜20%**変動します:\\n\\n1. **箱・冊子・カートン揃い**: 市場相場（Yahoo中央値）が基準（100%）\\n2. **箱あり・冊子なし**: 市場相場の85〜95%程度（85〜95%）\\n3. **箱なし・冊子あり**: 市場相場の85〜92%程度（85〜92%）\\n4. **箱なし・ラベル良好**: 市場相場の80〜90%程度（80〜90%）\\n5. **箱・冊子なし**: 市場相場の75〜85%程度（75〜85%）\\n\\nつまり、最大で**約25%（市場相場の25%程度）の差**が生じる可能性があります。"),
            ("なぜ外箱が重要なのか",
...
def render_label_yogore(b):
    ...
    return {
        ...
        "sections": [
            ...
            (f"ラベル状態別の査定額目安（{name}）",
             f"{name}の基準価格 市場相場（Yahoo中央値）に対し、ラベル状態でこう変動します:\\n\\n1. **新品同様**: 市場相場（100%）\\n2. **軽度の汚れ**: 市場相場の90%程度（90%程度）\\n3. **シミ・水濡れ跡**: 市場相場の80%程度（80%程度）\\n4. **破れ・欠け**: 市場相場の65%程度（65%程度）\\n5. **強い退色・大きな損傷**: 市場相場の50%程度（50%程度）\\n6. **ラベル剥がれ（残あり）**: 市場相場の40%程度（40%程度）\\n7. **ラベル完全消失**: 市場相場の20%程度（20%程度、ヴィンテージ価値あれば例外）"),
            ("ラベル損傷が起きやすい原因",
             "**主な原因**:\\n\\n1. **直射日光・蛍光灯** — 退色・脆化\\n2. **湿度の高い場所** — シミ・カビ\\n3. **段ボール直置き** — シミ・吸湿\\n4. **粘着テープの貼り付け** — 剥がし跡\\n5. **タンスや棚の埃** — 表面汚れ\\n6. **指で頻繁に触る** — 油脂による変色\\n7. **温度変化** — ラベルの剥がれ\\n8. **化学物質の接触** — 洗剤・芳香剤の蒸気"),
            ("ラベルを保護する7つの保管方法",
