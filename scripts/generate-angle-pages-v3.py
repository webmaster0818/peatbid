...
def render_ranking(b):
    ...
    return {
        ...
        "toc_label": "買取業者ランキング",
        "intro": f"{name}を高く売るには、業者選びが最重要。同じ市場相場クラスのボトルでも、業者により**数万〜数十万円**の査定差が生まれます。本記事ではPeatBid編集部が選定基準に基づいて選んだ{name}に強い買取業者4社を、ランキング形式で比較します。",
        "sections": [
            ("ランキングの選定基準",
...
def render_kihaku(b):
    ...
    return {
        ...
        "h1": f"{name}の希少性と投資価値",
        "toc_label": "希少性・投資価値",
        "intro": f"{name}は希少度{rarity_label}クラスのボトルで、現在の買取相場は実勢の中央値前後。投資対象として保有すべきか、それとも売却すべきか——コレクターが直面する判断を、データと市場動向から考察します。",
        "sections": [
            ("希少性のランク評価",
