...
def render_takaku_uru(b):
    ...
    return {
        ...
        "sections": [
            ...
            ("コツ3: 複数業者で相見積もり",
             f"同じ{name}でも、業者によって**数万〜数十万円**の査定差が生じます。1社だけで決めるのは絶対に避けるべきです。\\n\\n推奨フロー: (1)ヒカカク等の一括査定サイトで5社以上から見積もり取得、(2)お酒買取専門店（リカスタ等）に個別で2〜3社から見積もり、(3)合計7〜8社の見積もりを比較、(4)手取り額（手数料・送料込み）で最高値の業者を選定。\\n\\n査定無料・キャンセル無料の業者ばかりなので、複数比較のコストはゼロです。"),
            ("コツ4: 売却タイミングを見極める",
...
def render_ranking(b):
    ...
    return {
        ...
        "description": f"{name}に強い買取業者をランキング形式で徹底比較。ヒカカク・バイセル・リカスタの査定スピード・査定額・サポート・信頼性を多角的に評価し、目的別おすすめを提示。",
