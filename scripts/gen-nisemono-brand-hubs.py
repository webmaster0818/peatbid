...
FAQS = lambda n: [
    ...
    (f"{n}が本物か確認する一番確実な方法は？",
     f"最終的には専門家の確認が確実です。(1)お酒買取の専門店での無料鑑定査定、(2)メーカー公式お客様窓口へロット番号を伝えての確認、の2つが有効です。自分でできる一次チェックは、ラベル印刷・キャップ／封緘・液色と液面・底面刻印・購入経路の5点です（本記事で解説）。"),
...
def checks(n):
    return [
        ...
        ("不審なボトルを見つけた場合の対処法",
         f"自身が保有する{n}や購入検討中のボトルが不審な場合は、(1)ボトル全体・ラベル各角度・キャップ・底面刻印を高解像度で撮影、(2)お酒買取の専門店で無料鑑定査定、(3)メーカー・正規輸入元の公式窓口へロット番号を伝えて確認、(4)購入経路を遡り返品交渉、(5)詐欺の疑いがあれば警察へ相談、の順で対処してください。贋作と知りながらの売却は詐欺罪に該当し得るため、誠実な対処が重要です。"),
    ]
