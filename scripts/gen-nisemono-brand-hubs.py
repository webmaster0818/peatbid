...
def checks(n):
    return [
        ...
        ("チェック2：キャップ・封緘・ホログラム",
         "サントリー（山崎・響・白州）やマッカランなどはホログラム等の真贋要素が用いられます。本物のホログラムは見る角度で色が変化し立体感があります。贋作はホログラムが粗い・反射が均一・平面的・貼り方が雑、といった違和感が出やすいです。キャップの締まり具合・封緘の状態（隙間や不自然さ）も確認しましょう。"),
        ("チェック3：液色・液面の状態",
        ...
        ("不審なボトルを見つけた場合の対処法",
         f"自身が保有する{n}や購入検討中のボトルが不審な場合は、(1)ボトル全体・ラベル各角度・キャップ・底面刻印を高解像度で撮影、(2)お酒買取専門店（JOYLAB等）で無料鑑定査定、(3)サントリー・ニッカ等メーカー公式窓口へロット番号を伝えて確認、(4)購入経路を遡り返品交渉、(5)詐欺の疑いがあれば警察へ相談、の順で対処してください。贋作と知りながらの売却は詐欺罪に該当し得るため、誠実な対処が重要です。"),
    ]
