#!/usr/bin/env python3
"""
v2: Expanded brand-kaitori pages (10,000-12,000 chars each).
...
def state_table_rows(p):
    p = int(p)
    return [
        ("未開封・完璧", "箱・冊子・カートン揃い", fmt(int(p * 0.95)), fmt(p)),
        ("未開封・箱なし", "ラベル良好", fmt(int(p * 0.80)), fmt(int(p * 0.90))),
        ("未開封・冊子なし", "外箱あり", fmt(int(p * 0.85)), fmt(int(p * 0.95))),
        ("未開封・ラベル軽度汚れ", "付属あり", fmt(int(p * 0.75)), fmt(int(p * 0.88))),
        ("未開封・ラベル損傷", "付属あり", fmt(int(p * 0.60)), fmt(int(p * 0.75))),
        ("未開封・液面減少", "やや進行", fmt(int(p * 0.55)), fmt(int(p * 0.70))),
        ("開封済み・9割以上残", "付属あり", fmt(int(p * 0.30)), fmt(int(p * 0.40))),
        ("開封済み・半分以下残", "付属あり", fmt(int(p * 0.15)), fmt(int(p * 0.25))),
    ]
...
def render_page(b, all_brands):
    ...
    rows = state_table_rows(price)

    # Related brands (same category, sorted by price descending, take top 6 except current)
    related = sorted(
        [x for x in all_brands if x["category"] == category and x["slug"] != slug_base],
        key=lambda x: int(x.get("reference_price_jpy_2026_05") or 0),
        reverse=True,
    )[:6]
    related_links = "\n".join(
        f'            <Link href="/articles/{r["slug"]}-kaitori/" className="block bg-white border border-warm-border rounded-xl p-4 hover:shadow-md transition-shadow"><span className="text-xs text-amber-dark font-bold">関連銘柄</span><p className="text-sm font-bold mt-1">{r["name_ja"]} の買取相場</p><p className="text-xs text-warm-gray mt-1">参考 {fmt(int(r["reference_price_jpy_2026_05"]))}</p></Link>'
        for r in related
    )
    ...
    rows_html = "\n".join(
        f'                <tr><td>{r[0]}</td><td>{r[1]}</td><td><strong>{r[2]}〜{r[3]}</strong></td></tr>'
        for r in rows
    )
    ...
    faqs = [
        (f"{name}は今いくらで売れますか？",
         f"2026年5月現在、{name}（未開封・箱付き）の買取相場は約{fmt(price)}前後です。状態・付属品・タイミングにより上下します。最高値を狙うなら**複数業者で相見積もり**が鉄則です。"),
        (f"{name}を高く売るコツは？",
        ...
        (f"{name}は税金がかかりますか？",
         f"年間の売却益が50万円を超え、給与所得者の場合に他の所得と合わせて20万円を超えると、譲渡所得として確定申告が必要です。保有期間5年超の場合、長期譲渡所得として課税対象額が1/2に軽減されます。"),
        ...
        (f"{name}と他の銘柄、どっちを先に売るべき？",
         f"市場流動性・価格安定性・自身の保有目的を総合的に判断。希少度が高い銘柄ほど長期保有でプレミアが乗る傾向があるため、**急ぎでなければ希少銘柄は保有継続、流通量の多い銘柄から先に売却**するのが一つの戦略です。"),
    ]
...
export const metadata: Metadata = {{
  title: "{name}の買取相場【2026年最新】{fmt(price)}前後・状態別査定額と高く売る完全ガイド",
  description: "{name}（{name_en}）の最新買取相場、状態別査定額、蒸溜所の歴史、テイスティング、希少性、贋作リスク、おすすめ買取業者4社比較まで完全解説。コレクター・オーナー必読の決定版ガイド。",
}};
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <article className="prose">
          ...
          <p>2026年5月現在、{name}（未開封・箱付き）の買取相場は<strong>約{fmt(price)}前後</strong>。希少度は<strong>{rarity_label}</strong>クラスに位置し、{rarity_detailed}</p>
          ...
          <div className="table-wrapper">
            <table>
              <tbody>
                ...
                <tr><td><strong>参考買取価格（未開封・箱付き）</strong></td><td><strong>{fmt(price)}前後</strong></td></tr>
              </tbody>
            </table>
          </div>
...
{rows_html}
          ...
          <p>{fmt(price)}クラスの高額銘柄では、贋作・偽造ボトルのリスクが存在します。本物を見抜くには以下の5つのチェックが有効です:</p>
