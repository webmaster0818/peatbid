...
export const metadata: Metadata = {
  title: '竹鶴25年の偽物・贋作の見分け方【2026年9月最新】本物との違い5点（ラベル・キャップ・液面）と売却前チェック',
  description: '竹鶴25年の偽物・贋作を見分ける具体的なポイントを徹底解説。ラベル・キャップ・液色・ホログラム・購入経路の5要素から本物を確実に判定する方法と、被害時の対処法。',
};
...
export default function Taketsuru25NisemonoMikataPage() {
  return (
    <>
      <FaqSchema />
      <div className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <div className="article-hero mb-8">
          <Image src="/images/heroes/taketsuru-25.png" alt='竹鶴25年の偽物・贋作の見分け方' width={1200} height={440} className="w-full h-[220px] object-cover rounded-xl" priority />
          <div className="article-hero-overlay rounded-xl" />
        </div>

        <article className="prose">
          <h1 className="font-display text-3xl md:text-4xl font-semibold mb-2 !border-none !pb-0 !mt-0">竹鶴25年の偽物・贋作の見分け方</h1>
          ...
          <p>竹鶴25年のような高額ウイスキーは、贋作リスクが高まる銘柄です。特に未開封・箱付きで市場相場前後の流通価格を持つ銘柄は、海外を中心に巧妙な贋作が出回ることもあります。本記事では、竹鶴25年の偽物を見抜く5つのチェックポイントと、不審なボトルへの対処法を解説します。</p>
...
キャップの締まり具合・封蝋の状態も確認ポイントです。本物は精密に封蝋されているのに対し、贋作は封蝋に隙間や違和感があることが多いです。</li></ol>` }} />
          ...
          <div dangerouslySetInnerHTML={{ __html: `<p>本物の竹鶴25年は<strong>均一な琥珀色</strong>で透明感があります。贋作の特徴:</p><ol><li>色ムラ・濁り</li><li>沈殿物の存在</li><li>不自然に濃い/薄い色</li><li>気泡が永久に残る</li><li>振った時の液体の動きが不自然（粘度が異なる）

また、液面の高さも重要なチェックポイント。本物の竹鶴25年は通常、肩口（首の付け根）の少し下まで液体があります。極端に液面が下がっている場合は、長期保管による「目減り」か、贋作の可能性があります。</li></ol>` }} />
...
贋作と判明した場合、買取業者は買取を拒否します。また、贋作と知りながら売却することは詐欺罪に該当する可能性があるため、誠実に対処することが重要です。</li></ol>` }} />
          ...
          <div className="bg-burgundy/5 border-2 border-burgundy/30 rounded-xl p-6 my-8 not-prose">
            ...
            <h3 className="font-display text-xl font-semibold text-ink mb-3 !mt-0 !border-none">あなたの竹鶴25年、いま売るといくら？</h3>
            <p className="text-sm text-warm-gray leading-relaxed mb-4">✅ 本物と確認できたら：竹鶴25年の現在の実勢中央値は<strong className="text-ink">¥190,500</strong>（ヤフオク実落札 n=36件・毎週月曜更新・IQR外れ値除去後）です。保証額ではありませんが、売却判断の基準になります。価値が変わる前に、次の3ステップで納得して高く売りましょう。</p>
            <ol className="space-y-2 text-sm text-ink mb-4 list-none pl-0">
              <li><span className="font-bold text-burgundy">STEP 1</span>　相場の詳細を見る → <Link href="/articles/taketsuru-25-kaitori/" className="shingan-to-kaitori text-amber-dark underline">竹鶴25年の買取相場 完全ガイド</Link></li>
              <li><span className="font-bold text-burgundy">STEP 2</span>　高く売るコツ → <Link href="/articles/taketsuru-25-takaku-uru/" className="shingan-to-kaitori text-amber-dark underline">査定額を最大化する7つのコツ</Link></li>
