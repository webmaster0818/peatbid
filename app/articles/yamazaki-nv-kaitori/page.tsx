...
export const metadata: Metadata = {
  title: "山崎ノンエイジの買取相場【2026年最新】18,000円前後・状態別査定額と高く売る方法",
  description: "山崎ノンエイジ（Yamazaki NV）の最新買取相場と査定ポイント。サントリー山崎のノンエイジ。日本ウイスキーの代表的な定番品。状態別係数・付属品の影響・おすすめ買取業者まで完全ガイド。",
};
...
function FaqSchema() {
  return <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
    ...
    "@type": "FAQPage",
    "mainEntity": [
      { "@type": "Question", "name": '山崎ノンエイジは今いくらで売れますか？', "acceptedAnswer": { "@type": "Answer", "text": '2026年5月現在、山崎ノンエイジ（未開封・箱付き）の買取相場は約18,000円前後です。状態・付属品の有無により上下します。' } },
      { "@type": "Question", "name": '山崎ノンエイジを高く売るコツは？', "acceptedAnswer": { "@type": "Answer", "text": '(1)外箱・冊子等の付属品を揃える、(2)未開封のまま売る、(3)複数業者で相見積もりを取る、(4)直射日光を避けて縦置き保管、の4つが基本です。' } },
      { "@type": "Question", "name": '山崎ノンエイジの開封済みでも買取できますか？', "acceptedAnswer": { "@type": "Answer", "text": 'はい、開封済みでも買取可能ですが、未開封の20〜40%程度の査定額となります。残量により評価が変わります。' } }
    ]
  }) }} />;
}

export default function YamazakiNvPage() {
  return (
    <>
      <FaqSchema />
      <div className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <article className="prose">
          <h1 className="font-display text-3xl md:text-4xl font-semibold mb-2 !border-none !pb-0 !mt-0">山崎ノンエイジの買取相場と高く売る方法</h1>
          <p className="text-warm-gray text-sm mb-8">最終更新: 2026年5月12日</p>

          <p>サントリー山崎のノンエイジ。日本ウイスキーの代表的な定番品。本記事では、<strong>山崎ノンエイジ（Yamazaki NV）</strong>の最新買取相場・状態別査定額・付属品の影響・高く売るための具体策まで、PeatBid編集部の調査をもとに解説します。</p>

          <h2>山崎ノンエイジの銘柄プロファイル</h2>
          ...
          <p>ジャパニーズウイスキーは2010年代後半から世界的評価が一気に高まり、特に長期熟成・終売銘柄は二次流通市場で高値が定着しています。現行品として流通しているため、需給は安定的。ただしプレミア化が始まっているケースも多く、定期的なチェックが重要です。</p>

          <h2>山崎ノンエイジの状態別 買取相場</h2>
          ...
          <p>山崎ノンエイジの現在の買取相場は、海外オークション（Sotheby&apos;s、Bonhams、Whisky Auctioneer等）の落札データと国内ジャパニーズウイスキー市場の需給で決まります。<strong>現行品として流通しているため、需給は安定的。ただしプレミア化が始まっているケースも多く、定期的なチェックが重要です。</strong></p>

          <p>特に未開封・箱付き・付属品揃いのコレクター品は、市場流通量が限られるため二次流通価格が継続的に高水準で推移しています。</p>

          <h2>山崎ノンエイジを高く売る4つのポイント</h2>
          ...
          <h2>山崎ノンエイジに関するFAQ</h2>

          <div className="space-y-3 not-prose">
            {[
              { q: '山崎ノンエイジは今いくらで売れますか？', a: '2026年5月現在、山崎ノンエイジ（未開封・箱付き）の買取相場は約18,000円前後です。状態・付属品の有無により上下します。' },
              { q: '山崎ノンエイジを高く売るコツは？', a: '(1)外箱・冊子等の付属品を揃える、(2)未開封のまま売る、(3)複数業者で相見積もりを取る、(4)直射日光を避けて縦置き保管、の4つが基本です。' },
              { q: '山崎ノンエイジの開封済みでも買取できますか？', a: 'はい、開封済みでも買取可能ですが、未開封の20〜40%程度の査定額となります。残量により評価が変わります。' },
            ].map((faq) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 not-prose">
            ...
            <Link href="/articles/yamazaki-55-kaitori/" className="block bg-white border border-warm-border rounded-xl p-4 hover:shadow-md transition-shadow"><span className="text-xs text-amber-dark font-bold">銘柄ガイド</span><p className="text-sm font-bold mt-1">山崎55年の買取相場</p></Link>
          </div>
