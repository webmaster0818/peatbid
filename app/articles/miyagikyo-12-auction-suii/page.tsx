...
export const metadata: Metadata = {
  title: '宮城峡12年のオークション相場推移と買取価格【2026年8月最新】国内外データ分析',
...
export default function Miyagikyo12AuctionSuiiPage() {
  return (
    <>
      <FaqSchema />
      <div className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <div className="article-hero mb-8">
          <Image src="/images/heroes/miyagikyo-12.png" alt='宮城峡12年のオークション相場推移' width={1200} height={440} className="w-full h-[220px] object-cover rounded-xl" priority />
          <div className="article-hero-overlay rounded-xl" />
        </div>

        <article className="prose">
          <h1 className="font-display text-3xl md:text-4xl font-semibold mb-2 !border-none !pb-0 !mt-0">宮城峡12年のオークション相場推移</h1>
          ...
          <p>宮城峡12年の国内買取相場は、海外オークションの落札結果に強く連動します。本記事では、Sotheby's、Bonhams、Whisky Auctioneer等の主要オークションで取引された価格データから、相場の動向と今後の見通しを分析します。</p>
          ...
          <div dangerouslySetInnerHTML={{ __html: `<p>宮城峡12年の二次流通価格は、過去5年で大きく上昇しています。
...
この5年間で累計<strong>約3〜5倍</strong>の上昇を示しており、ウイスキー投資としての魅力が証明されています。</li></ol>` }} />
          ...
          <div dangerouslySetInnerHTML={{ __html: `<p>宮城峡12年クラスの銘柄が取引される主要オークション:</p><ol><li><strong>Sotheby's（ニューヨーク・香港・ロンドン）</strong> — 月次〜四半期ごとに開催、プレミアム銘柄中心。最高クラスの希少銘柄が出品される。</li><li><strong>Bonhams（香港・ロンドン）</strong> — 同様にプレミアム銘柄。アジア富裕層が主要バイヤー。香港オークションは特に活発。</li><li><strong>Whisky Auctioneer（UK）</strong> — 月次オンラインオークション、中位銘柄まで幅広く扱う。透明性が高い。</li><li><strong>Just Whisky Auctions（UK）</strong> — UK中心、月次開催、コレクター向け。</li><li><strong>Whisky Hammer（UK）</strong> — 月次オンライン、新興プレイヤー。</li><li><strong>Acker Wines（香港・ニューヨーク）</strong> — 富裕層会員制、超高額ボトル中心。</li></ol>` }} />
...
同じ宮城峡12年でも、地域によって5〜15%の価格差が生じます。海外オークション経由の方が高値が期待できる場合が多く、輸送・関税を考慮しても国内売却より有利なケースもあります。</li></ol>` }} />

          <h2 id="section-5">6. 今後の見通し（中長期）</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p>宮城峡12年の中長期見通しは、以下の3要因が支え、<strong>高値推移が予想</strong>されます:</p><ol><li>世界的需要の継続（コレクター層拡大）</li><li>供給制限（長期熟成銘柄の物理的限界）</li><li>新興市場の参入（インド、中東、東南アジア）
