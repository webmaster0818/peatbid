...
export const metadata: Metadata = {
  title: 'マッカラン25年の歴史・特徴と買取価値【2026年8月最新】なぜ高い?市場評価の根拠',
  description: 'マッカラン25年の歴史的背景、スペイサイドのストーリー、味わいの特徴、世界的評価、市場価値が高い理由を専門ライターが徹底解説。コレクター・購入検討者必読の銘柄完全ガイド。',
};
...
export default function Macallan25RekishiPage() {
  return (
    <>
      <FaqSchema />
      <div className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <div className="article-hero mb-8">
          <Image src="/images/heroes/macallan-25.png" alt='マッカラン25年の歴史と特徴' width={1200} height={440} className="w-full h-[220px] object-cover rounded-xl" priority />
          <div className="article-hero-overlay rounded-xl" />
        </div>

        <article className="prose">
          <h1 className="font-display text-3xl md:text-4xl font-semibold mb-2 !border-none !pb-0 !mt-0">マッカラン25年の歴史と特徴</h1>
          ...
          <div className="bg-cream/30 border border-warm-border rounded-xl p-4 mb-6 not-prose">
            ...
            <p className="text-xs text-warm-gray mt-1">スコッチウイスキー / スペイサイド / 25年熟成 / 希少度 ハイ / 市場相場 200,000円</p>
          ...
          <div className="bg-cream/40 border border-amber/30 rounded-xl p-5 mb-8 not-prose">
            ...
            <ol className="space-y-1.5 text-sm text-amber-dark">
              <li><a href="#section-0" className="hover:underline">1. マッカラン25年の市場ポジションとデータ分析</a></li>
              <li><a href="#section-1" className="hover:underline">2. スペイサイドの創業と歴史</a></li>
              <li><a href="#section-2" className="hover:underline">3. マッカラン25年の誕生背景</a></li>
              <li><a href="#section-3" className="hover:underline">4. マッカラン25年の味わいプロファイル</a></li>
          ...
          <p>マッカラン25年はスコッチウイスキーを代表する銘柄の1つで、スペイサイドが手がける25年熟成のボトル。本記事では、銘柄の誕生背景、蒸溜所のストーリー、味わいの個性、世界的評価、そして二次流通市場で高値が定着している理由を解説します。</p>

          <div className="bg-cream/40 border-l-4 border-amber rounded-r-xl p-4 my-6 not-prose">
            <p className="text-xs text-amber-dark font-bold tracking-wider mb-1">この銘柄の市場データ</p>
            <p className="text-sm text-ink leading-relaxed">直近180日のYahoo!オークションではマッカラン25年の落札が約146件確認でき、落札額の中央値は200,000円でした。取引が活発で換金しやすい銘柄で、本記事の査定目安はこの実勢中央値を基準に算出しています。</p>
          </div>
          ...
          <h2 id="section-0">1. マッカラン25年の市場ポジションとデータ分析</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p><strong>マッカラン25年</strong>はスコッチウイスキー（スペイサイド）の25年熟成、度数43%、希少度は希少に分類される銘柄です。</p><p>直近180日の実勢中央値は<strong>200,000円</strong>（流通サンプル146件、当サイト独自集計）。価格帯としては<strong>高額帯</strong>にあたり、業者間の査定差が金額として大きく表れるため、相見積りの効果が高い価格帯です。</p><p>流通量は<strong>標準的</strong>の水準です。一定の流通量があり相場が形成されやすいため、複数業者の比較で適正額を見極めやすい銘柄です。</p><p>スコッチは蒸溜所・地域（スペイサイド）やボトリング、熟成年数で評価が大きく分かれます。同じ年数でも蒸溜所の人気度で査定差が出るため、銘柄固有の相場を踏まえた業者選びが有効です。</p><p>※ 数値は当サイトがYahoo!オークションの過去180日落札データ（IQRで外れ値除去）から集計した参考値で、買取額を保証するものではありません。</p>` }} />

          <h2 id="section-1">2. スペイサイドの創業と歴史</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p>スペイサイドはスコッチウイスキー業界で長い歴史を持つ蒸溜所/メーカー。創業以来、伝統的な製法と現代の品質管理を融合し、マッカラン25年を含む数々の名作を世に送り出しています。</p><p>ジャパニーズウイスキーの場合は1923年の山崎蒸溜所創設以降、スコッチの場合は18〜19世紀に遡る歴史を持つ銘柄も多数あります。各蒸溜所が独自の蒸溜方法・水・樽選びで個性を表現し、それが今日の市場価値につながっています。</p>` }} />

          <h2 id="section-2">3. マッカラン25年の誕生背景</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p>マッカラン25年は、スペイサイドの長年の研究と職人技術の結晶。25年熟成の長期にわたる樽熟成を経て、樽材から溶け出した成分と原酒が複雑に交差し、唯一無二の味わいが生まれます。</p><p>発売当時はスコッチウイスキーの品質を世界に示すフラッグシップ的位置付けで、限定的な生産量と高い品質基準により、コレクターや愛好家から絶大な支持を集めました。</p>` }} />

          <h2 id="section-3">4. マッカラン25年の味わいプロファイル</h2>
...
これら全ての要素が組み合わさり、{age_label}の時間をかけて醸成されたのが{name}の味わいです。</li></ol>` }} />
          ...
          <div dangerouslySetInnerHTML={{ __html: `<p>マッカラン25年を含むスペイサイドの銘柄は、以下の国際品評会で度々受賞しています:</p><ol><li>ISC（インターナショナル・スピリッツ・チャレンジ）</li><li>ワールド・ウイスキー・アワード（WWA）</li><li>サンフランシスコ・ワールド・スピリッツ・コンペティション</li><li>ジム・マーレイ氏のウイスキー・バイブル</li><li>ウイスキー・マガジン・アワード

この国際的評価が、二次流通市場での高値を支えています。受賞歴・専門家評価が「投資する価値のあるボトル」という認識を醸成しているのです。</li></ol>` }} />

          <h2 id="section-5">6. 二次流通市場での位置付け</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p><strong>Sotheby's、Bonhams、Whisky Auctioneer</strong>等の海外オークションで、マッカラン25年は定期的に取引されており、落札データが国内買取相場の指標となっています。</p><p>特に終売や限定リリースの場合は、希少性プレミアが大きく乗ります。コレクター・投資需要が三つ巴で価格を支え、長期的な値上がり期待を生んでいます。</p><p>アジア富裕層・欧米コレクター・国内コレクターが市場参加者として競合する中、マッカラン25年の価値は中長期で見て高水準を維持する見通しです。</p>` }} />

          <h2 id="section-6">7. コレクター市場での需要動向</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p>マッカラン25年の需要は以下の層から支えられています:</p><ol><li><strong>アジア富裕層</strong>（中国・香港・台湾・シンガポール）— ジャパニーズウイスキーブームの主要バイヤー</li><li><strong>欧米コレクター</strong>（米・英・独）— Sotheby's・Bonhamsの主要顧客</li><li><strong>国内コレクター・愛好家</strong>— SNS・ウイスキーバー文化の普及で増加中</li><li><strong>投資家・ファンド</strong>— 「液体資産」としての保有が増加

この多様な需要構造が、マッカラン25年の市場価値を安定的に支えています。</li></ol>` }} />

          <h2 id="section-7">8. 今後の市場見通し</h2>
          <div dangerouslySetInnerHTML={{ __html: `<p>マッカラン25年を含むスコッチウイスキーの中長期見通しは、(1)世界的需要の継続、(2)供給制限、(3)新興市場の参入 の3要因が支え、<strong>高値推移が予想</strong>されます。</p><p>ただし短期は需給の振れにより上下動も発生します。「自分にとっての納得価格になったら売る」のが実践的な判断軸です。</p><p>長期保有を視野に入れる場合は、適切な保管と保険加入を検討し、5〜10年スパンでの値上がり期待を持つのが現実的です。</p>` }} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 not-prose">
            ...
            <Link href="/articles/macallan-25-auction-suii/" className="block bg-white border border-warm-border rounded-xl p-4 hover:shadow-md transition-shadow"><span className="text-xs text-amber-dark font-bold">オークション推移</span><p className="text-sm font-bold mt-1">マッカラン25年のオークション推移</p></Link>
